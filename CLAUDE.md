...
- **⚠️ `deploy-pages` can fail transiently on the large artifact.** With ~53k files the Pages `syncing_files` phase occasionally returns *"Deployment failed, try again later"* (seen 2026-07-03; intermittent — the same artifact deploys fine most nights). It's **retriable** and a failed deploy does **not** take down the previous one (the site keeps serving the last good deploy). Both `nightly.yml` (deploy job) and `deploy.yml` now do **one automatic retry** of `deploy-pages` after a 45s pause, so the unattended nightly self-heals. If it ever fails *both* attempts, just re-run the deploy job (`gh run rerun <id> --failed`). Extra reliability margin available by dropping the ~22k low-value domain-alias pages (`build_entity_pages.py --no-domain-aliases`) to roughly halve the file count.
